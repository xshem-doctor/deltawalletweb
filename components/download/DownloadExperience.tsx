// components/download/DownloadExperience.tsx

"use client";

import { CheckCircle2, Download, X } from "lucide-react";
import DeltaButton from "@/components/shared/DeltaButton";
import { siteConfig } from "@/config/site";
import { useState } from "react";

type DownloadExperienceProps = {
  buttonLabel: string;
};

export default function DownloadExperience({ buttonLabel }: DownloadExperienceProps) {
  const [open, setOpen] = useState(false);

  function handleDownloadClick() {
    setOpen(true);
  }

  return (
    <>
      <DeltaButton
        href={siteConfig.apkUrl}
        onClick={handleDownloadClick}
        className="h-14 w-full px-8 text-base md:w-auto"
      >
        <span className="inline-flex items-center gap-2">
          <Download className="h-5 w-5" />
          {buttonLabel}
        </span>
      </DeltaButton>

      {open ? (
        <>
          <button
            aria-label="Close download message"
            className="fixed inset-0 z-[90] bg-black/70"
            onClick={() => setOpen(false)}
          />

          <div
            role="dialog"
            aria-modal="true"
            className="
              fixed inset-x-4 bottom-4 z-[100]
              rounded-3xl border border-white/10 bg-delta-nearBlack p-6 shadow-card
              md:left-1/2 md:top-1/2 md:bottom-auto md:w-full md:max-w-md
              md:-translate-x-1/2 md:-translate-y-1/2
            "
          >
            <button
              onClick={() => setOpen(false)}
              className="absolute end-5 top-5 flex h-9 w-9 items-center justify-center rounded-full border border-white/10 text-delta-lightGray/70 hover:border-delta-orange hover:text-white"
              aria-label="Close"
            >
              <X className="h-4 w-4" />
            </button>

            <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-delta-orange/10">
              <CheckCircle2 className="h-7 w-7 text-delta-orange" />
            </div>

            <div className="mt-5">
              <h3 className="text-xl font-semibold text-white">
                Thanks for downloading Delta Wallet
              </h3>

              <p className="mt-3 text-sm leading-6 text-delta-lightGray/65">
                The APK download should start now. After installation, open the app and create your account.
              </p>
            </div>

            <div className="mt-5 rounded-2xl border border-delta-orange/20 bg-delta-orange/10 p-4">
              <p className="text-sm leading-6 text-delta-lightGray/70">
                If Android asks for permission, allow installation from your browser, then continue installing Delta Wallet.
              </p>
            </div>

            <button
              onClick={() => setOpen(false)}
              className="mt-5 h-12 w-full rounded-full bg-delta-orange text-sm font-semibold text-delta-black hover:brightness-110"
            >
              Got it
            </button>
          </div>
        </>
      ) : null}
    </>
  );
}